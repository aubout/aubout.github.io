                    ...
                    if (d.length >= set.minimumLength)
                    {
                         ...
                         else
                         {
                              for (var i = 0; i < tipuesearch.pages.length; i++)
                              {
                                   ...
                                   if (tipuesearch.pages[i].text.normalize('NFD').replace(/[\u0300-\u036f]/g, "").search(pat) != -1)
                                   {
                                        var m_c = tipuesearch.pages[i].text.normalize('NFD').replace(/[\u0300-\u036f]/g, "").match(pat).length;
                                        score += (20 * m_c);
                                   }
                                   if (tipuesearch.pages[i].tags)
                                   {
                              ...
                              for (var i = 0; i < found.length; i++)
                              {
                                   if (l_o >= start && l_o < set.show + start)
                                   {
                                        ...
                                        {                                        
                                             ...
                                             {
                                                  d_w = d.split(' ');
                                                  var s_1 = found[i].desc.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, "").indexOf(d_w[0]);
