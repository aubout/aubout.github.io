...
{var m_c=tipuesearch_in.pages[i].text.normalize("NFD").replace(/[\u0300-\u036f]/g, '').match(pat).length;score+=(20*m_c);}
...
if(d_w[f].match('^-'.normalize("NFD").replace(/[\u0300-\u036f]/g, ''))||d_w[f].match('^-'))
{pat=new RegExp(d_w[f].substring(1),'i');if(tipuesearch_in.pages[i].title.search(pat)!=-1||tipuesearch_in.pages[i].text.search(pat)!=-1||tipuesearch_in.pages[i].text.normalize("NFD").replace(/[\u0300-\u036f]/g, '').search(pat)!=-1||tipuesearch_in.pages[i].tags.search(pat)!=-1)
{score=0;}}}
if(score!=0)
...
{var t=found[i].desc;if(set.showContext)
    {d_w=d.split(' ');
if(d.normalize("NFD").replace(/[\u0300-\u036f]/g, '').normalize("NFC") == d)
    {var s_1=found[i].desc.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, '').normalize("NFC").indexOf(d_w[0]);if(s_1>set.contextStart){var t_1=t.substr(s_1-set.contextBuffer);var s_2=t_1.normalize("NFD").replace(/[\u0300-\u036f]/g, '').normalize("NFC").indexOf(' ');t_1=t.sunstr(s_1-set.contextBuffer+s_2);t_1=$.trim(t_1);if(t_1.length>set.contextLength){t='... '+t_1;}}}
    {var k_1=found[i].desc.toLowerCase().indexOf(d_w[0]);if(k_1>set.contextStart)
{var t_1=t.substr(k_1-set.contextBuffer);var k_2=t_1.indexOf(' ');t_1=t.substr(k_1-set.contextBuffer+k_2);t_1=$.trim(t_1);if(t_1.length>set.contextLength)
    {t='... '+t_1;}}};}
if(standard)
{d_w=d.split(' ');for(var f=0;f<d_w.length;f++)
{if(set.highlightTerms)
{if(d.normalize("NFD").replace(/[\u0300-\u036f]/g, '').normalize("NFC") == d)
    {var wt = s_t.substr(s_t.normalize("NFD").replace(/[\u0300-\u036f]/g, '').normalize("NFC").indexOf(d_w),d.length);
    t=t.replace(wt,"<h0011>" + wt + "<h0012>")}
    {var patr=new RegExp('('+d_w[f]+')','gi');t=t.replace(patr,"<span class=\"tipue_search_content_bold\">$1</span>")}}}}
